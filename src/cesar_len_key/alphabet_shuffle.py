def DivisorsList(x: int) -> list[int]:
  """
  Does:
...
def RemadeKey(key: str, max_len: int) -> list[int]:
  """
  Does:
  ...
  numbed_key: list[int] = [numbed_seq[sorted(key).index(char)] for char in key]
...
def PiecewiseShuffledAlphabet(key: str, alph: str) -> list[str]:
  ...
  alph_divs = DivisorsList(len(alph))

  # Means: ключ, содержащий в себе только числа, в порядке символов
  numbed_key = RemadeKey(key, len(alph))
  ...
  listed_alph = [alph[i : i + needed_div] for i in range(0, len(alph), needed_div)]
  ...
  for i in range(len(listed_alph)):
    listed_alph[i] = "".join(
      listed_alph[i][numbed_key[j]] for j in range(len(listed_alph[i]))
    )

  return listed_alph


def ShuffledAlphabet(key: str, alph: str) -> str:
  """
  Does:
  ...
  listed_alph = PiecewiseShuffledAlphabet(key, alph)
  ...
  shuffled_alph = [x for _, x in sorted(zip(numbed_key, listed_alph, strict=False))]
